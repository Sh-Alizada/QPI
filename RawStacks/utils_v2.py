...
@timer
def get_stack_and_t(ome_tiff_path):
    """
    ...
    # image_stack = imread(ome_tiff_path)
    # Open the OME-TIFF file
    with TiffFile(ome_tiff_path) as tif:
        image_stack = tif.asarray()
        ome_metadata = tif.imagej_metadata['time_stack']

    
    time_stack = ast.literal_eval(ome_metadata)

    return image_stack, time_stack
...
def validate_filter(value, name):
    ...
    if isinstance(value, list):
        if len(value) != 2 or not all(isinstance(i, (int, np.integer)) for i in value):
            raise ValueError(f"{name} list must contain exactly two integers [min_{name}, max_{name}].")
    elif value is not None and not isinstance(value, (int, np.integer)):
...
@timer
def save_ome_tiff(file_path, image_array, compression='zstd', metadata=None):
