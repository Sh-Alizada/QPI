...
TIMEOUT = 30  # Seconds to wait for a missing frame

MAX_CONCURRENT_TASKS = 10  # Limit the number of concurrent positions being processed per frame
...
# Load existing timestamps for a given pos
def load_timestamp_info(pos):
    timestamp_file = os.path.join(savefolder, f'{fstart}pos{pos}_timestamps.json')
    if os.path.exists(timestamp_file):
        with open(timestamp_file, 'r') as f:
            return json.load(f)
    return {}

# Save frame status (processed or skipped) in the JSON file
def save_frame_status(pos, frame, status, reason=None, timestamp=None):
    """Save the status of a frame (processed or skipped) in the JSON file."""
    # Create a unique JSON file for each position
    timestamp_file = os.path.join(savefolder, f'{fstart}pos{pos}_timestamps.json')
    
    # Load existing data
    data = load_timestamp_info(pos)
    
    # Update the data with the new frame status
    key = f"frame{frame}"
...
def matlab_image_processing(folder_path):
    ...
    return Phase, Absorption, timestamp
...
async def process_frame_for_position(pos, frame):
    ...
    async with semaphore:
        ...
        # Load the current timestamp info for this pos
        timestamp_info = load_timestamp_info(pos)

        # Check if this frame has already been processed or skipped
        if f"frame{frame}" in timestamp_info:
            print(f"Frame {frame} for pos {pos} already processed or skipped. Skipping...")
            return  # Skip if the frame has already been processed or marked as skipped

        # Polling mechanism with a timeout: wait until the frame folder exists or timeout occurs
        start_time = time.time()
        while not os.path.exists(folder_path):
            elapsed_time = time.time() - start_time
            if elapsed_time > TIMEOUT:
                print(f"Frame {frame} for pos {pos} did not appear. Skipping after {TIMEOUT} seconds.")
                save_frame_status(pos, frame, status="skipped", reason="timeout")
                return  # Skip this frame if it doesn't appear within the timeout period
            print(f"Waiting for pos {pos}, frame {frame} to appear... ({elapsed_time:.1f} seconds elapsed)")
            await asyncio.sleep(2)  # Wait for 2 seconds before checking again

        # If the frame folder appears, proceed with processing
        print(f"Processing pos {pos}, frame {frame}...")

        # Offload MATLAB processing to a separate thread but reuse the MATLAB engine
        loop = asyncio.get_event_loop()
        Phase, Absorption, timestamp = await loop.run_in_executor(executor, matlab_image_processing, folder_path)
        ...
        overwrite_plane(phase_filename, frame - 1, Phase)
        overwrite_plane(absorption_filename, frame - 1, Absorption)

        # Save processed status and timestamp in the JSON file for the specific pos
        save_frame_status(pos, frame, status="processed", timestamp=timestamp)

        # # Delete the frame folder to save space
        # if os.path.exists(folder_path):
        #     shutil.rmtree(folder_path)  # Deletes the directory and all its contents
        #     print(f"Deleted {folder_path}")

        print(f"Overwritten pos {pos}, frame {frame} in OME-TIFF and timestamp saved")
