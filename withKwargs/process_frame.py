...
# @my_decorators.timer
def process_frame(file_path, common_back, conv_fac_to_rads=(4 * np.pi) / 32767,
                  bckg_params=None, watershed_params=None, imageprops_params=None, image_mat_key='Phase', time_mat_key='timestamp'):
    """
    ...
    raw_phase_image, matlab_serial_time = utils.load_mat_file(file_path, [image_mat_key, time_mat_key]).values()
    
    print(f"loaded file: {file_path}")
    
    frame_time = utils.matlab_serial_to_datetime(matlab_serial_time.item())
    
    if raw_phase_image.std() == 0:  # Skip if blank image
        return None, None, None, frame_time
    ...
    return int16_image, uint16_label, frame_props_df, frame_time
